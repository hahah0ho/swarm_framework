...
class CentralOrchestrator:
    ...
    def update_agent_state_and_result(self, agent: Agent, state: str, result: Any = None):
        ...
        print(f"[Swarm] Agent {agent.name} state updated to {state}.")
        if result:
            print(f"[Swarm] Agent {agent.name} result: {result}")

    def monitor_agents(self):
        """
        에이전트 상태를 모니터링하고 실패한 에이전트를 추적.
        """
        for agent_name, state in self.agent_states.items():
            print(f"[Monitor] Agent {agent_name} is in state: {state}")
            if state == "Failed" and agent_name not in self.failed_agents:
                self.failed_agents.append(agent_name)

    def retry_failed_agents(self, agents: List[Agent], messages: List):
        """
        실패한 에이전트 재시도.
        """
        retry_agents = [agent for agent in agents if agent.name in self.failed_agents]
        if retry_agents:
            print("[Orchestrator] Retrying failed agents...")
            retry_results = self.swarm.run_parallel_agents(retry_agents, messages)
            for result, agent in zip(retry_results, retry_agents):
    ...
    def get_agent_result(self, agent_name: str) -> Any:
        ...
        Args:
            agent_name (str): 결과를 가져올 에이전트 이름.

        Returns:
            Any: 에이전트의 작업 결과.
        """
        return self.agent_results.get(agent_name, None)